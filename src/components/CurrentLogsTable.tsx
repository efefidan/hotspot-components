...
const CurrentLogsTable: React.FC = () => {
  ...
  const content = translations[language]?.currentLogsTable || translations["en"].currentLogsTable;
  ...
  return (
    <div className="mt-8 p-4 bg-white shadow-md rounded-lg border border-gray-200">
      ...
      <div style={{ overflowX: "auto", width: "100%" }}>
        ...
        >
          <thead>
            <tr className="bg-gray-100 text-gray-500" style={{ fontSize: "14px", textAlign: "left" }}>
              {[
                { label: content.hotelName, field: "hotelName" },
                { label: content.logName, field: "logName" },
          ...
          <tbody className="text-gray-500">
            {data.map((item, index) => (
              <tr key={index} className="hover:bg-gray-100 transition duration-150" style={{ borderBottom: "1px solid #e2e8f0" }}>
                ...
                <td style={{ padding: "10px", width: columnWidths.date }}>
                  {new Date(item.date).toLocaleDateString(language) + " - " + new Date(item.date).toLocaleTimeString(language, { hour: "2-digit", minute: "2-digit" })}
                </td>
