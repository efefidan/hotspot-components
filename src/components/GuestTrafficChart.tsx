...
import translations from "@/locales/translations"; // Çeviriler
...
const GuestTrafficChart = () => {
  ...
  const content = translations[language]?.guestTrafficChart || translations["en"].guestTrafficChart;
    ...
    <div className="bg-white p-4 rounded-lg shadow-md flex flex-col md:flex-row items-center md:justify-between border border-gray-200">
      <div className="w-full  md:w-3/4">
        <div className="flex items-center mb-4">
          <h2 className="text-lg font-semibold px-2">{content.title}</h2>
          <select
            value={timeRange}
            onChange={(e) => setTimeRange(e.target.value)}
            className="border border-gray-300 rounded-md p-1 text-gray-500"
          >
            <option value="10 min">{content.timeOptions["10 min"]}</option>
            <option value="30 min">{content.timeOptions["30 min"]}</option>
            <option value="60 min">{content.timeOptions["60 min"]}</option>
          </select>
        </div>
        <Line data={getData()} options={options} />
      </div>
      <div className="w-full md:w-1/4 flex flex-col space-y-4 mt-4 md:mt-0 ">
        <InfoCard icon={<FaDownload />} label={content.download} value={`${getAverage(getData().datasets[0].data)} ${content.unit}`} percentage="9.2%" />
        <InfoCard icon={<FaUpload />} label={content.upload} value={`${getAverage(getData().datasets[1].data)} ${content.unit}`} percentage="9.2%" />
        <InfoCard icon={<FaMicrochip />} label={content.processor} value={`63.3 ${content.unit}`} percentage="9.2%" />
        <InfoCard icon={<FaMemory />} label={content.memory} value={`63.3 ${content.unit}`} percentage="9.2%" />
      </div>
    </div>
  );
};
